...
function selectRandom(arrRandom) {
    ...
    return arrRandom[Math.round(Math.random()*((arrRandom.length-1)))]
}
...
function revealMystery(envelope) {
    const result = `${envelope.suspect.firstName} ${envelope.suspect.lastName} killed Mr. Boddy using the ${envelope.weapon.name} in the ${envelope.room.name}`
    return result
}
